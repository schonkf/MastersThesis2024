...
def extract_appusage(df, windows, timestamp):
    ...
    for window_name, window_size in windows.items():
        ...
        windowed_data = before_esm[before_esm.index >= (timestamp - window_timedelta)]
        app_interacitons = 0
        for event_type in windowed_data.type:
            app_interacitons += 1
        windowed_appevent[f'{window_name}_number_of_app_interactions'] = app_interacitons
...
#                 else:
            ...
            # Append the windowed features dictionary to the list
...
def all_participants(all_participants_data, esm_responses, processing_functions, windows):
    all_windowed_data_list = []  # List to store dictionaries of windowed features
